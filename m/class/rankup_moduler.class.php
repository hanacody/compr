...
class rankup_moduler {
	...
	function set_modules() {
		...
		if(is_array($module_names)) {
			foreach($module_names as $folder) {
				$init_file = $mobile->m_dir.$folder.'/module.init.php';
	...
	function get_url($rows) {
		...
		//$url = $cs_rows['file'] ? array_shift(explode('|', $cs_rows['file'])) : $cs_rows['url'];
		return $mobile->m_domain.str_replace($mobile->m_folder.'/', '', $url);
	}
}
