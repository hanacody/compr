...
var rankup_post = {
	...
	search_post: function(dong) {
		...
		proc.parameters({mode: 'search_post', dongmyun:dong});
		proc.process(function(trans) {
			...
			for(var i=0; i<items.length; i++) {
				...
				new_div.innerHTML = '<a onClick="rankup_post.select_post(this, \''+ zipcode +'\', \''+ address +'\')">['+zipcode+']<span style="margin-left:6px">'+ sido +' '+ sigugun +' '+ dongmyun +' '+ bunji +'</span></a>';
	...
	// 주소검색창 열기
	open_post: function(obj, handler, zone) {
		blind.draw();
		this.frame = $(obj);
		this.frame.show();
