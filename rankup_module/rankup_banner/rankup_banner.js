...
var BANNER = function() {
	...
	this.settingFrm = null;		// 배너 설정폼
	this.bannerBody = null;		// 배너 바디 테이블
	this.bannerItems = null;		// 배너 아이템 테이블
	this.selClass = "selClass";	// 선택한 배너의 스타일
	this.selObject = null;			// 선택한 배너객체
	this.selRow = null;			// 선택한 배너의 줄 번호
	this.selNo = null;				// 선택한 배너의 인덱스 번호
	this.selBind = null;			// 선택한 배너의 바인드 번호
	this.maxBind = 100;			// 바인드 가능 갯수
	this.maxWidth = null;		// 배너 가로사이즈 제한
...
BANNER.prototype.select_item = function(arg) {
	...
	try {
		...
		var click_no = el.parentNode.getElementsByTagName("input")[0].value; // 배너 인덱스 번호
	...
	if(el.className != classObj.selClass) {
		...
		el.className = classObj.selClass; // 선택한 배너 재설정
...
BANNER.prototype.get_object = function(target) {
	...
	var target_bind = '';
	var target_row = 0;
	// 배너 리스트 중에서 몇번째 테이블의 몇번째 아이템인지 체크
...
BANNER.prototype.set_direction = function(el, mode) {
	setTimeout(function() {
			...
			//###############################################################
				...
				if(classObj.selRow==0) {
					if(select['obj'].getElementsByTagName('input')[1].value=='') {
						...
						// 최상위에서 바인드된 배너를 벗어나려고 할 경우
						select['obj'].rows[select['row']].getElementsByTagName('input')[1].value = ''; // 바인드 번호 제거
					...
					// 이웃하는 바인드 배너에 포함하지 않을 경우
					if(target['bind'] && (target['body']==select['body'] || (target['body']!=select['body'] && confirm("이웃하는 랜덤배너 그룹에 포함하시겠습니까?"+SPACE)))) {
						// 배너영역 초기화
						classObj.reset_media_contents();
						// 타겟에 선택된 배너 추가
						select['obj'].rows[select['row']].getElementsByTagName('input')[1].value = target['bind']; // 바인드 번호 부여
						copyNode = select['obj'].rows[select['row']].cloneNode(true);
						select['obj'].deleteRow(select['row']);
		...
		switch(select['obj'].rows.length) {
			...
			case 1: // 바인드가 풀린 경우에 해당
				select['obj'].getElementsByTagName('input')[1].value = ''; // 바인드 번호 제거
				select['obj'].parentNode.parentNode.getElementsByTagName('span')[0].innerHTML = ''; // 도구 제거
				break;
		}
...
BANNER.prototype.bind_item = function() {
	...
	if(datas.length<2) {
		alert("랜덤배너로 설정할 배너를 2개 이상 선택하여 주십시오."+SPACE);
		document.body.focus();
		return false;
	}
	if(!confirm("선택하신 배너를 랜덤배너로 설정하시겠습니까?"+SPACE)) {
		document.body.focus();
		return false;
	}
	...
	if(newBind == null) {
		alert("랜덤배너로 설정할 수 있는 최대 갯수( "+this.maxBind+"개 )를 초과하였습니다."+SPACE);
		document.body.focus();
		return false;
	}
	// 선택한 배너 복사
	...
	for(var i=0; i<this.bannerItems.length; i++) {
		...
		for(var j=0; j<trs.length; j++) {
			if(in_array(trs[j].firstChild.getElementsByTagName("input")[0].value, datas)==false) continue;
			trs[j].getElementsByTagName('input')[1].value = newBind; // 바인드 번호 할당
	...
	new_cell.innerHTML = this.get_template(bind_items, true); // 바인드
	document.body.scrollTop = 0;
	this.rebuild_event(true);
}

// 바인드 해제 - 현재 위치를 기준으로 해제
BANNER.prototype.resolve_item = function(el) {
	if(!confirm("선택하신 그룹의 랜덤배너 설정을 해제하시겠습니까?"+SPACE)) {
		document.body.focus();
		return false;
	}
	...
	for(var row=obj.rows.length-1; row>=0; row--) {
		obj.rows[row].getElementsByTagName("input")[1].value = ''; // 바인드 값 제거
...
BANNER.prototype.change_form = function(el) {
	...
	// 미디어등록 방식 폼
		...
		//form.address.setAttribute("required", "required"); - 2009.03.06 수정
	...
	switch(mode) {
		...
		case "insert":
			...
			if(this.selNo==null) this.bannerBody.insertRow(0).insertCell(0).innerHTML = this.get_template("<tr><td>"+banner_item['item']+"</td></tr>");
				...
				// 선택된 배너가 있을때 - 선택된 배너 아래에 추가
				this.selObject.className = ''; // 기존선택된 배너 초기화
...
BANNER.prototype.post_attached = function(el) {
	if(el.getAttribute("filter")!=null) {
		...
		if(in_array(exts[exts.length-1], filters)==false) {
			alert("배너로 등록할 수 없는 미디어 파일형식입니다."+SPACE);
			document.body.focus();
			return false;
		}
	}
...
BANNER.prototype.set_banner_outline = function() {
	...
	//for(var i=0; i<media.length; i++) media[i].style.border = media[i].style.border=="" ? "#555555 1px solid" : ""; // 토글
...
// 선택된 배너 다중 처리(사용/미사용, 새창/본창, 삭제) - Ajax 처리
	...
	setTimeout(function() {
		...
		switch(mode) {
			...
			case "view": // 사용값 변경시
				...
				if(data.length<1) {
					alert("사용여부를 변경할 배너를 선택하여 주십시오."+SPACE);
					document.body.focus();
					return false;
				}
			...
			case "delete": // 삭제시
				var data = (multi===true) ? classObj.get_checkAll() : classObj.selNo;
				if(data.length<1) {
					alert("삭제하실 배너를 선택하여 주십시오."+SPACE);
					document.body.focus();
					return false;
				}
			...
			case "target": // 새창모드 변경시
				...
				if(!modeValue) {
					alert("창모드를 변경할 수 없는 배너형태 입니다."+SPACE);
					document.body.focus();
					return false;
				}
		...
		if(!confirm("선택하신 배너"+modes[mode]+" 하시겠습니까?"+SPACE)) {
			document.body.focus();
			return false;
		}
		// Ajax 처리
				...
				if(resultData.getAttribute("result").match("success")) {
					...
					switch(mode) {
						...
						case "delete": // 삭제된 아이템 리스트에 반영
							...
							classObj.save_settings(true); // 순위/바인드정보 갱신
							break;
						case "outline": // 테두리표시 - 2010.12.20 added
							var obj = $(classObj.selObject).select('span[id="media"]')[0];
