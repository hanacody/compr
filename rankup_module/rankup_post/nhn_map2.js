...
var nmap = {
	...
	initialize: function(frame, map_key, nonclick) {
		...
		this.configs = {
			...
			minMaxLevel : [1, 14],
			size : new nhn.api.map.Size(this.frame.getDimensions().width-4, this.frame.getDimensions().height-4)
		}
	...
	get_geocode: function(string, single) {
		var self = this;
		if(string==undefined) var query = single ? $F('addrs1') : ($F('addrs1')+' '+$F('addrs2')).trim();
		else var query = string;
		proc.parameters({ mode: 'get_geocode', query: query, key: this.mapKey, coord: this.coord, encording: this.encoding, qty: 1 });
		proc.process(function(trans) {
			var item = trans.responseXML.getElementsByTagName('item')[0];
			if(item) {
				var infos = {};
				$w('mapx mapy').each(function(field) { infos[field] = parseFloat(item.getElementsByTagName(field)[0].firstChild.nodeValue) });
				var oPoint = self.get_point(infos.mapx, infos.mapy);
				self.set(oPoint, true);
			}
			else if(string==undefined) {
				string = query.split(' ');
				string.pop();
				self.get_geocode(string.join(' '));
			}
		}, false);
	}
}
