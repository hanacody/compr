...
class rankup_frame extends rankup_util {
	...
	function print_frames($entry, $no='', $depth='', $used='') {
		...
		if($entry['use_gnb']) $stacks[] = "use_gnb='yes'";
		if($entry['use_lnb']) $stacks[] = "use_lnb='yes'";
	...
	function _f15($bind) {
		...
		if($mpid) $rows['mselect'] = $mpid == $rows['no']	? $on_select : "";
		...
		if($rows['depth']<3) $rows['on_button'] = $on_button;

		// 첨부파일
		if($attaches && isset($on_attach)) {
			if($attach_kind=='lnb_title') {
				if($attaches[$rows['no']]) {
					$name = $attaches[$rows['no']];
					$rows['on_attach'] = fetch_skin(compact('name'), $on_attach);
				}
			}
			else if($attach_kind=='lnb_flash') {
				if($attaches[$rows['no']]['file']) {
					$rows['width'] = $attaches[$rows['no']]['width'];
					$rows['height'] = $attaches[$rows['no']]['height'];
					$name = $attaches[$rows['no']]['file'];
					$rows['on_attach'] = fetch_skin(compact('name'), $on_attach);
				}
			}
		}
	...
	function save_frame() {
		if($_POST['kind']=='new') { // 추가
			if($_POST['no']) { // 2~3차 메뉴
				...
				$bundle = $this->queryR("select if(max(bundle) is null, 1, max(bundle)+1) from $this->frame_table");
				$depth = 1;
				$position = 0;
				$parents = '';
			}
