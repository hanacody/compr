...
var rankup_frame = {
	...
	open: function(el, kind) { // 등록/수정폼 로드
		...
		$w('extra_frame lnb_frame gnb_frame').each(function(frame) {
		...
		$w('gnb_frame').each(function(frame) {
			var state = ((kind=='new' && depth>=2) || (kind=='edit' && depth>2));
			$(frame)[state?'hide':'show']();
			$A($(frame).select('input')).each(function(item) { item.disabled = state });
		});
		...
		proc.process(function(trans) {
			...
			$w('base_name extra_name target access_level page_type module options page_body_content link url used use_gnb use_lnb').each(function(field) {
	...
	template: {
		...
		<tr no="#{no}" class="#{parents}t#{no}" id="d#{depth}">\
			...
			<td align="left">\
				<a onClick="rankup_frame.open(this, \'edit\')"><img src="./img/btn_edit_s.jpg" align="absmiddle" alt="수정" /></a><a onClick="rankup_frame.del(this)"><img src="./img/btn_del_s.jpg" align="absmiddle" hspace="1" alt="삭제" /></a><a onClick="menu_handler(#{no})"><img src="./img/btn_preview_s.jpg" align="absmiddle" hspace="5" alt="미리보기" /></a>#{on_button}\
			</td>\
		</tr>'),
		on_button: '<a onClick="rankup_frame.open(this)"><img src="./img/btn_regist_s.jpg" align="absmiddle" alt="하위메뉴등록" /></a>'
	},
	save: function() {
		...
		proc.process(function(trans) {
			...
			infos['on_button'] = (infos.depth>2) ? '' : self.template.on_button; // 등록버튼
	...
	set_link: function() {
		...
		var bgColors = {1: '#e6f1fb', 2: '#fff', 3: '#fcf1e4'}; // 배경색
